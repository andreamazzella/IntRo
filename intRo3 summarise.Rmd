...
For this topic, we'll use `trial`, a sample dataset from package `gtsummary`. This contains the baseline characteristics of 200 patients who received two different cancer drugs (`trt`) and treatment outcomes: `response` and `death`.
Check its documentation to learn more.

```{r}
# trial$trt <- factor(trial$trt)
# trial$response <- factor(trial$response, labels = c("no", "yes"))
# trial$death <- factor(trial$death, labels = c("no", "yes"))
...
*Want to know more?*
This only works well if the variables are coded in the correct data classes, so it will not work well with datasets that have not been cleaned first. That's what those earlier `factor()` functions were doing. See `intRo5 clean.Rmd` for how to do this.
...
# 2. Bar plot
ggplot(trial, aes(y = stage)) + geom_bar()
```
...
summarytools::ctable(trial$trt, trial$response, useNA = "no")
...
*Exercise*
Write some code to calculate the percentage of people with `grade` I cancer who were alive at the end of the study.
(If you can't remember the names of your variables, you can run `names(trial)`.
```{r ex contingency}
# With `tabyl`
...
We can use as many of these as you want in a single `summarise()` call, and you can also summarise more than one variable at the same time.
`n()` will count the observations in that group.
...
Make a Box plot to show the distribution of age across the two treatment groups in the `trial` dataset.
