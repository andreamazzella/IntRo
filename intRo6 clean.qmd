...
```{r}
falls_v2 <- rio::import("data/S-Fall for analysis new final FINAL.xlsx", skip = 3)
...
We need to remove all these rows. Before reading the chunk below, can you remember at least one function to conditionally keep rows? (topic 4)
...
Sometimes there can be an error where dates are read in the format Excel uses under the hood (days since 01/01/1900). To fix this, we can use this handy function from `lubridate`, `excel_numeric_to_date()`:
...
We can change everything to lower-case using `tolower()`, and then use `recode()` to fix the remaining inconsistencies.

Of note, in `recode` the *old* values go to the *left*, without quotes, and the *new* values go to the *right*, in quotes.
...
*Exercise 8* Let's correct this value. Note that you need backticks to identify the old number to recode.
...
falls_r1 |> mutate(hosp_cbt = tolower(hosp_cbt) |>
                              recode(declined = "no",
                                     ys = "yes",
                                     `only 1 session` = "yes") |> 
                              factor())
...
```{r challenge_exercise_solution}
# Import and explore
bristol <- rio::import("data/bristol.csv")
...
# Export
# bristol_cleaned |> rio::export("data/bristol_cleaned.dta")
```
