...
To create an R script, you may:

-   Go to the toolbar: `File` → `New File` → `R Script`
-   Or, go to the Files tab in the bottom right of the screen and click on the `New Blank File` icon → `R Script` (this will automatically save the file in the folder you see)
...
-   **Text**, which is the main component.
    -   This can be formatted *as if it's a text editor.* If you're using Visual editing, you will find a tool bar with formatting options. If you're using Source, you need to know a syntax called Markdown.
...
You can tell R that something has a name; R will remember it and you can use that later in your code. This is called *assignment* and is done by using the arrow `<-` (keyboard shortcut: `Alt` + `-` on Windows, `⌥` + `*` on Mac).

Assignment creates a new "object". The code in the exercise below means: "radius" gets the value of 5.7. "radius" is a new object.
...
2.  Write a line of code using function *log* to calculate the logarithm of 100.
