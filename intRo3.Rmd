...
```{r}
diab <- read_dta("diab_stata.dta") %>% mutate_if(is.labelled, as_factor)
diab
```
...
In the chunk above there's a `%>%`. What's that?

Pipes are operators that make your code quicker to write and easier to read. They're *optional*: base R doesn't have them, but they can make your R easier. We're going to use two pipes: `%$%` and `%>%`.
