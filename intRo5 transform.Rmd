...
library(ggplot2)   # graphs (part of the tidyverse)
library(forcats)   # dealing with factors (part of the tidyverse)
```
...
2. You use a format-specific function to load the data, and assign it to a name of your choice.

For example:

1. Make sure that this `.Rmd` file is saved in the same folder as a `data` folder containing `diabetes.rds`, a simulated datasets.
...
(We'll cover other data formats in the next topic).
...
Now we can calculate each observation's age as of 07 May 2020.

Dealing with dates in all programming language is tricky, because they can be listed in many different formats, with different delimiters between numbers. Package `lubridate` (great name!) has some good functions to help with dates.
...
```{r}
dm <- dm |> mutate(bmi_group = cut(bmi_dc,
                               breaks = c(0, 18.5, 25, 30, +Inf),
                               labels = c("underwt (<18.5)", "normal wt (18.5-25)", "overwt (25-30)", "obese (>30)")))
...
There's another way of doing this, if you don't want to keep the original variable – recoding values (we'll explore this in a future topic).
...
# Export
# bristol |> saveRDS("bristol_v2.rds")
```
