...
2.  You use a function to load the data. `rio::import()` can read many formats, including `.csv`, Excel spreadsheets, Stata datasets, and the R format, `.rds`.
...
2.  Then we use `rio::import()` to load the file.

```{r}
# Data import
dm <- rio::import("data/diabetes.rds") 
```
...
# Regroup
dm <- dm |> mutate(
  macrocontinent = fct_collapse(continent,
...
dm <- dm |> mutate(
  frailty = case_when(disability == "severe" ~ "severely frail",
...
`rio::export()` lets you save in many formats - you just need to specify the file extension after the file name.

```{r}
# dm |> rio::export("data/diabetes_edited.rds")
```
...
```{r solution_recap}
# Import
bristol <- rio::import("data/bristol.rds")
...
# Export
# bristol |> rio::export("data/bristol_v2.rds")
```
