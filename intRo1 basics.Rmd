...
Some sample datasets are available in R. Let's import "women", a small dataset containing the heights and weights of 15 women, into our Environment. (It's a default R dataset, so R already knows what it is)
...
*Exercise*
Import "chickwts", another default dataset containing information on chicken feeds and weights.
```{r}
...
*Recap exercise*
Use the women dataset and calculate the mean height in metres. There are a few ways of coding this and the choice is yours :)
(1 inch = 0.0254 m)
...
# Recap exercise
women$height                  #  To check in which unit was height measured

# Multiple ways of doing this calculation
# Option 1: all in one line
mean(women$height) * 0.0254

# Option 2: break it down
mean_in <- mean(women$height) # calculate the mean height in inches and assign this

mean_mt <- mean_in * 0.0254   # convert into metres and assign this

mean_mt                       # visualise result
```
