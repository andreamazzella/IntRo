...
*Exercise 8.* Let's use another dataset, `chickwts`. This contains info on chicken weights (in grams) and their feed type.
...
In R, a **vector** is a list of objects of the same type, in a specific order. For example, 5 2 4 is a vector (of numbers); "black" "blue" "green" is a vector (of characters).
...
Some functions will accept vectors:
```{r}
log(c(100, 10))
```
