...
```
The average creatinine at 2 weeks is 113 mmol/L in the control group, and 87 mmol/L  in the single-dose group. In this sample, there's a difference of 26 mmol/L. How do we know if this reflects a true difference in the whole population, and is not due to chance?
...
"We can be 95% confident that the mean true population mean of creatinine at 2 weeks among patients treated with a single LAmB dose lies between 84 and 90".
...
The other important output is the 95% CI of this difference: (22, 31). We can be 95% confident that the true difference in creatinine average is between 22 and 31 mmol/L.
...
amb %>% ggplot(aes(x = age, y = crea_0)) +
  geom_point() +
  geom_smooth(method = "lm") # lm stands for linear
```
...
- The intercept estimate (In this case, 68 mmol/L)
- the age coefficient estimate (In this case, +0.04)
...
creatinine = -0.04 * age + 68

This means that for every year of life, creatinine increases by 0.04 mmol/L.

How confident are we about this statement? Not confident at all: the p value is 0.46. We can say that there is no evidence against the null hypothesis.

(The intercept, 68, mathematically refers to the mean creatinine at age 0 years. But in this dataset the youngest person is 18, so that value is purely extrapolated by continuing the regression line to the left. It does not have any clinical meaning.)
