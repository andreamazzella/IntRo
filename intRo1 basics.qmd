...
Some `data.frame`s are included in R, so we can use them to practise.

For example, let's explore a small table of patient-level data on bacteraemias. For the moment, run this code without focussing on the `read.csv` function - we'll cover it in a later session.
...
bacteraemias <- read.csv("./data/bacteraemias.csv")
...
*Exercise 8.* Let's use another dataset, `metabolic.csv`. This contains info on some patient factors and outcomes for metabolic disease.
...
```{r}
# Load the data
metabolic <- read.csv("./data/metabolic.csv")
...
```{r}
# Import
aggregated_results <- read.csv("./data/aggregated_results.csv")
