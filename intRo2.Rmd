---
title: "2: Data import and ggplot"
...
For example, make sure the `diab.csv`, `diab_excel.xlsx`, and `diab_stata.dta` files are in the same folder as this .Rmd. Then run this chunk:
```{r message=FALSE, warning=FALSE}
# Comma-separated value files
diab_csv <- read_csv("diab.csv")
...
*Exercise*
Insert a new chunk and use it to import the `poo.csv` dataset.
```{r message=FALSE}
poo <- read_csv("poo.csv")
```
...
For example, in order to simply visualise the dataset:
```{r}
...
ggplot(diab, aes(bmi, fill = exercise)) +                       # stratify
  ...
  labs(title = "Body Mass Index distribution",
       x = "BMI (kg/m²)",
       y = "Frequency") +
...
*Recap exercise*
- Make a new histogram of a continuous variable of your choice, in how many bins you want.
...
ggplot(drop_na(diab, religion), aes(fct_infreq(religion))) +
  ...
  labs(title = "Religion and continent of origin",
       subtitle = "A colourful bar chart",
       x = "Religion",
       y = "Frequency") +
  theme_minimal() +
