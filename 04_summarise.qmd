...
-   Their first argument is always a dataframe.
-   The output is always a new dataframe; the original data frame is left unchanged.
...
Many functions from modern R packages list the dataframe as their first argument. This includes key functions from `ggplot2`, `dplyr`, `janitor`. This means that you can use the pipe:
...
trial |>
  ...
  # Add counts back in, before percentages 
...
# ...is equivalent to:
adorn_ns(adorn_pct_formatting(adorn_percentages(tabyl(trial, trt, response), "row")), position = "front")
...
adorn_ns(tab_trial_perc_formt, position = "front")
...
For example, to calculate the mean `ttdeath` we can run:
...
2.  Why do think you get that result?
...
trial |>
  summarise(
    n(),
    avg_follow_up = mean(ttdeath, na.rm = T),
    .by = c(trt, death)
    )
```
...
NB: if there are many columns and you're using visual editor, the output will be scrambled. Try running the code from the console instead.

#### Exercise 7

-   Use `summary()` with your dataframe.
...
> *Want to know more?* This function doesn't provide a helpful summary of variables containing text, like `trt`. Once you specify the data class, the output will be more useful; see `08_clean.qmd` for how to do this.
