...
For this session, we'll use some antimicrobial resistance data adapted from the ESPAUR 2024 report.
...
# Preview of dataset
amr_pseud_acinetobac

# RStudio Viewer
View(amr_klebsiella)

# Structure
glimpse(amr_ethnicity)
```
...
ggplot(amr_pseud_acinetobac, aes(x = organism, y = perc_resistant, fill = antimicrobial)) +
  ...
  labs(title = "Antibiotic resistance in Acinetobacter and Pseudomonas",
       x = "Organism",
       y = "Percentage of isolates that are resistant")
```
...
Show the two sexes with different *shapes*, rather than different colours.

```{r}
# TODO
ggplot(amr_klebsiella, aes(x = age, y = CrCl, shape = sex)) +
  geom_point()
```
...
*Tip*: you can reverse engineer what a line does by deleting it and see what happens to the output.
...
- the last plot you've generated - into the current directory; if you're running code from a qmd, this will be the same directory as the .qmd file). However, I recommend you save all your graphs in an output folder.
...
```{r}
#| label: recap exercise solution

# Q1
min(bacteraemias$wbc)

# Q2
# You can do that with Ctrl Shift I (if you've set up this keyboard shortcut) or via the toolbar > Code > Insert chunk.
...
ggplot(amr_ethnicity, aes(x = ethn_group, y = median_age)) +
...
ggplot(amr_pseud_acinetobac, aes(x = antimicrobial, y = perc_resistant, fill = organism)) +
  geom_col(position = position_dodge(0.8), width = 0.7)
```
