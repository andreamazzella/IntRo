...
```
You might have noticed some warnings. Disregard them for now, and try summarising the new dataset the way we did for the .dta file. You can also use `View()` to look at the data.

*Exercise 1*
How many major differences can you spot between the two summaries?
...
These are stored as factors. Factors have "levels" indicated by integers (1, 2, 3...) and corresponding "labels" which contain text. Functions `as.factor()` and `factor()` change the class into a factor.
...
# Method 2 (base R)
# dm_new$diabetes[dm_new$diabetes == "no-"] <- "no"
