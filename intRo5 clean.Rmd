...
So far we've worked on the `diabetes.dta` dataset. Now imagine that data collection has progressed, and now there are some new variables.
...
```{r}
# Data import
dm_new <- readr::read_csv("data/diab_new.csv")

# Old dataset
dm_stata <- read_stata("data/diabetes.dta") %>% mutate_if(is.labelled, as_factor)
```
...
It looks like `...18`, `...19` and `test_delete` don't contain any data:
```{r}
dm_new %>% select(c(...18, ...19, test_delete))
```
...
# Import and explore
poo <- readr::read_csv("data/poo.csv")
poo
