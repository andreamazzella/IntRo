...
`dplyr` is a package that makes manipulating data easier. We used its functions `count()`, and `summarise()` in topic `04_summarise`.

In this topic, we'll focus on two main tasks for which this package is helpful: reducing rows (`filter()`) and reducing columns (`select()`). We'll use the in-built dataset `msleep` containing information on sleep of 83 mammal species.
