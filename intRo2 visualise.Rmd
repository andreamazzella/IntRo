...
Package `ggplot2` can make stunning graphs – plotting is one of the areas in which R shines.
...
1. The thick _central line_ indicates the _median_
2. The _lower and upper hinges_ indicate the _25th and 75th percentile_ respectively - so the distance between them is the interquartile range (IQR)
3. The _upper whisker_: extends to the largest value no further than _1.5×IQR_ from the hinge

If there are any data points that are beyond the whiskers, they will be individually displayed as _dots_.
...
- Make a new boxplot of a continuous variable of your choice.
...
Vectors are often used when we want to apply a function to more than one object, or when the argument of a function needs to contain more than one element.
```{r}
# A vector of numbers
c(6, 7, 8)

# A vector of names
c("cirrhosis", "fibrosis")
