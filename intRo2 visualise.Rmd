...
*Exercise* Install package "readxl" (NB: the last character is a lowercase L, not a digit 1.)
```{r}
...
For example, make sure that this .Rmd file is saved in the same folder as a "data" folder containing `ambition.csv`, `diab_excel.xlsx`, and `diab_stata.dta` files (They are simulated datasets containing the same data). Then run this chunk:

```{r message=FALSE, warning=FALSE}
# Comma-separated value files
ambition <- read_csv("data/ambition.csv") # NB: the location HAS TO be in quotes

# Excel files
syph <- read_excel("data/syphilis.xlsx")

# Stata files
diab <- read_dta("data/diabetes.dta")
diab <- mutate_if(diab, is.labelled, as_factor) # disregard for now (it simply applies labels to values)
```
You will note that now these three datasets have been imported in your Environment, in the top right of your window.
...
*Exercise*
This chunk won't execute. Can you spot the *four* bugs that break the code, and fix them so that you can have a histogram _with a title_?
(Hint: RStudio will help with some of these bugs. Look at the left of the chunk).
```{r bugs}
