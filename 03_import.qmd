...
-   `dbplyr` to simplify querying a SQL database table.
...
However, if you need to keep your data on a shared drive, you need to specify the longer path. I recommend using the full path to identify the data folder (e.g., `//FILECOL17.phe.gov.uk/ProjectData/HCAI/intRo/data/`) and not just e.g. `H:/intRo/data/`, because some people might not have mapped the same drive to the same letter. You can then use `paste0()` or `glue::glue()` to identify your file. Please note, I recommend **against** using `setwd()` to do this. More information on data location will be covered in session `13_workflows`.
...
ambi_raw <- read_csv("data/ambition.csv")
glimpse(ambi_raw)
```
...
```{r}
smoking <- read_csv("data/metabolic.csv", col_select = c(patient_id, smoker))
```
...
Please note that if you're downloading a delimited file from a shared drive, your internet download speed will be a limiting factor.
...
*Exercise 2*. Read `diabetes.rds`, a file saved in the `/data` folder.
...
```{r}
candi_site5 <- read_excel("data/candidaemias.xlsx")
```
...
# TODO: move to solutions
help(read_excel)
candi_antifung <- read_excel("data/candidaemias.xlsx", sheet = "meds")
```
...
```{r}
# TODO: move
pps_raw <- read_dta("data/point_prevalence.dta")
pps_raw
```
...
```{r}
pps <-
    read_dta("data/point_prevalence.dta") |> 
...
Whatever you do to the dataset, it doesn't change the original file. If you want to make some changes permanent, you need to use a writing function. Important: never overwrite the initial, raw data file.

### Exporting as csv

You can use `readr::write_csv`.

```{r}
# dm |> write_csv("data/diabetes_edited.csv")
```
...
*Exercise X*. Create a separate connection to another database within a server you have access to.

## Pull data from a SQL table

You can get data from a SQL database table in a variety of ways:

-   Use package `dbplyr` to query a database using `dplyr` code - detailed below
-   Write the SQL query in a SQL chunk within a qmd - detailed below
-   Write the SQL query as a character string in `DBI::dbGetQuery()`
-   Call a SQL query (e.g. `query.sql`) from within R code by using `readr::read_lines()` and then passing it to `DBI::dbGetQuery()`
...
2.  Use `dplyr` functions to query the database. This will be covered in session `06_subset`, but for now just be aware that `select` keeps only the columns listed, and drops all other columns. Be aware that this still is not a dataframe, it's just a SQL commands. In the background, `dbplyr` is converting the `dplyr` code into a SQL query. You can see the SQL query by using `show_query` on the new object.

```{r}
sicbl_name_lookup_db <- sicbl_db |>
    select(SICBL24CD, SICBL24NM)
...
### Using SQL chunks within a qmd - IMO we don't need this, the above is enough

If you're working from a qmd script (like this one), you can use a SQL chunk, that lets you write the SQL query and run it directly from this script.

A SQL chunk needs to know the name of the connection object. You do this by typing it in the `connection` chunk option, i.e., what comes after `#| connection:`.

```{sql}
#| connection: con_LookupsShared

SELECT *
FROM LookupsShared.dbo.vLKP_SICBL24
```

(NB: In session `10_sql` we'll cover more complex SQL queries)

You can assign the results of your query to an R object by using chunk option `output.var`.

```{sql}
#| connection: con_LookupsShared
#| output.var: lookup_sicbl

SELECT *
FROM LookupsShared.dbo.vLKP_SICBL24
```

You can then use this object in the rest of your R code. Remember to close the connection once you don't need it anymore.
