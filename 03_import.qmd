...
```{r}
pps |> write_csv("data/processed/point_prevalence.csv")
```
...
*Exercise 5*. Save the `pps` data frame as an RDS file in the "processing" folder.
...
*Exercise 6*. Create a separate connection to another database within a server you have access to.
...
-   Write the SQL query in a SQL chunk within a qmd - detailed below (but we may want to remove this)
...
### Using SQL chunks within a qmd - IMO we don't need this, the above is enough at this stage as it gets the job done
...
*Stretch exercise*. If you have access to LookupsShared:
...
pps |> write_rds("data/processing/point_prevalence.rds")
```
...
esp |>
    ggplot(aes(Age_Band_Min, Population)) +
