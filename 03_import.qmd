...
Important: never overwrite the initial, raw data file. Ideally, separate the data in different folders according to their type: raw files, processing tables, processed tables.
...
```{r}
dm |> write_csv("data/processed/diabetes_edited.csv")
```
...
```{r}
# dm |> write_rds("data/processed/diabetes_edited.rds")
```
